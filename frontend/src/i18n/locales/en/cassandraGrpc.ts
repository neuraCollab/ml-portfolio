export const cassandraGrpc = {
  workspace: {
    ...
    description:
      'A real distributed ML serving system: a FastAPI gateway routes every request through a ' +
      'Coordinator pod, which discovers real Kubernetes worker-pod replicas and round-robin ' +
      'dispatches gRPC calls between them, backed by Apache Cassandra for both data storage and ' +
      'cross-pod model persistence. The ML workload running through this pipeline is a topic ' +
      "classifier distilled from AutoTopic's slower unsupervised BERTopic clustering into a fast, " +
      'real-time-servable TF-IDF + Logistic Regression model -- the NLP methodology and model-quality ' +
      "work live on AutoTopic's own page; this project is about the serving architecture around " +
  ...
  staticResults: {
    eyebrow: 'Results',
    title: 'Real Results: Serving Pipeline + ML Workload',
    distributedNote:
      "The distributed-system behavior this project demonstrates -- real Coordinator routing, pod discovery, horizontal scaling, and fault-tolerant retries -- is shown live above (Architecture, System Status) and validated by the 55/55 regression tests below. The numbers here describe this specific ML workload's own result: useful context for judging the pipeline, not the main point of the project.",
  ...
  errorAnalysisSection: {
    ...
    intro:
      "This is ML workload analysis, not the project's central concern -- the point of this page is the distributed serving system around this classifier, not the classifier's own quality (see AutoTopic for that work). It's included because a serving pipeline honest about its own limitations, including a mediocre workload, is more credible than one that hides them.",
  ...
  regressionTestsSection: {
    ...
    intro:
      "A real, passing test suite across every layer of the distributed system is part of what makes this project's engineering credible, not just its architecture diagram: cassandra-grpc-ml/worker/tests/ (test_ml_core.py, test_model_store.py) and cassandra-grpc-ml/coordinator/tests/ (test_dispatch.py, test_k8s_client.py, test_app.py) are the project's existing real tests for the pure ML logic, model persistence, and the Coordinator's pod-discovery/dispatch/routing logic -- run here, not just described. backend/tests/test_cassandra_grpc_*.py cover the API schemas, ingestion, and the backend-to-Coordinator HTTP proxying logic.",

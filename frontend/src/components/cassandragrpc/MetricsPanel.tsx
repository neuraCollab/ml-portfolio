    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
      ...
      {!checked ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <MetricCard label={t('cassandraGrpc.metrics.accuracyLabel')} value={`${(metrics.accuracy * 100).toFixed(1)}%`} icon={Target} color="text-cyan-300" />
          <MetricCard label={t('cassandraGrpc.metrics.macroPrecisionLabel')} value={metrics.macroPrecision.toFixed(3)} icon={Target} color="text-cyan-300" />
          <MetricCard label={t('cassandraGrpc.metrics.macroRecallLabel')} value={metrics.macroRecall.toFixed(3)} icon={Target} color="text-cyan-300" />
          <MetricCard label={t('cassandraGrpc.metrics.macroF1Label')} value={metrics.macroF1.toFixed(3)} icon={Target} color="text-cyan-300" />
          <MetricCard label={t('cassandraGrpc.metrics.microF1Label')} value={metrics.microF1.toFixed(3)} icon={Target} color="text-cyan-300" />

...
The demo's Results sections show real numbers from real runs (checked-in, not recomputed on
load). The interactive tabs (live training/inference, live Kubernetes scaling) call a real
backend and need it running locally -- see [Limitations](#limitations).

## At a glance

- **373,657** real, unlabeled user chat/prompt logs clustered end to end, no manual labeling
  (AutoTopic)
- **3 reasoning LLMs × 4 KV-cache quantization schemes**, 80 real math problems, with a
  reproduced tensor-level failure mechanism (quantization study)
- Real **AD8232 + Arduino + Raspberry Pi 5** hardware rig, **5.18ms** CPU-only inference per
  prediction (ECG Edge AI)
- Real Kubernetes worker pool, measured **87 → 346 req/s** scaling 1→5 replicas, **63/63
  automated tests** passing (Cassandra + gRPC ML)

## Projects

| Project | Problem / Goal | What I built | Result |
|---|---|---|---|
| **[KV-Cache Quantization × Reasoning Study](kv-cache-quantization-reasoning-study/README.md)** | Does KV-cache quantization break LLM reasoning, and why? | Quantized 3 reasoning LLMs across 4 schemes on 80 real math problems; captured per-layer K/V tensors under a teacher-forced replay | DeepSeek-R1-Distill collapses to 0% accuracy under FP8-E4M3; a same-size Qwen3 model keeps ~53.7% (vs. 52.5% baseline) |
...
| **[ECG Edge AI](raspberry-pi-ecg/README.md)** | Run a multi-class ECG classifier on real sensor hardware, fully offline | Wired 2 AD8232 sensors to a Raspberry Pi 5, reconstructed a 6-lead ECG from 2 channels, trained a 19-class TorchScript classifier | 5.18ms CPU-only inference per prediction, no cloud round-trip |
| **[Cassandra + gRPC ML](cassandra-grpc-ml/README.md)** | Serve an ML classifier reliably across a scalable worker pool | Built a gRPC Coordinator + Kubernetes worker pool (pod discovery, round-robin dispatch, MinIO model registry) around a distilled classifier | 87 → 346 req/s scaling 1→5 replicas; automatic recovery after pod failure; 63/63 tests passing |
| **[RL Autopilot](rl_cv_car-autopilot/README.md)** *(WIP)* | Fuse camera/LiDAR/IMU into an RL driving policy on KITTI | Custom Gym environment + SAC policy over multi-modal KITTI data | Still in active development -- not yet a completed case study |
...
quantized KV-cache to capture real per-layer, per-channel K-tensor statistics.
...
command against the real model shows the tensor-level mechanism: a small number of outlier
K-channels carry a disproportionate share of the quantization noise, because a per-tensor FP8
scale is set by the single largest-magnitude value in the tensor.
...
dataset). BERTopic's topic diversity (0.966) is marginally higher than the LDA-10 baseline
(~0.91); LDA-10 topic stability passed on aggregate (mean Jaccard 0.376 > the 0.3 threshold) but
4 of its 10 topics fell below that threshold individually.
...
**What I learned.** Most portfolio ML projects start from a CSV; this one starts at the
electrodes, with real analog signal noise and lead-off artifacts to handle before a line of
PyTorch runs.
...
**Problem.** Serve an ML classifier reliably across a horizontally-scaled worker pool, not just
as a single in-process model call.

**Approach.** Distilled AutoTopic's slow BERTopic clustering into a TF-IDF + LogisticRegression
classifier, then built a real Kubernetes worker pool around it: a Coordinator that discovers
Ready pods via the k8s API and round-robins gRPC calls across them, live pod-failure injection
relying on Kubernetes' own Deployment self-healing (no custom reconciliation logic), and a MinIO
...
- **Main web app** (AutoTopic, ECG, Cassandra + gRPC ML frontend/backend): the `docker compose`
  command above is enough for AutoTopic and ECG.
...
- One FastAPI backend adapts each project's existing Python code -- it doesn't reimplement the
  ML logic.
- Cassandra + gRPC ML is the one project actually distributed across real, separate processes
  (Coordinator + worker pods on Kubernetes); AutoTopic and ECG run in-process inside the backend.
- The KV-cache quantization study sits outside this app entirely, as an independent research
  repository.
